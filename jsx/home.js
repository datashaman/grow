  ...
  var Schedule = React.createClass({
    ...
    render: function() {
      var types = _.map(data.glyphicons, function(icon, name) {
        ...
        var active = pos !== -1;
        return <button key={name} type="button" onClick={this.handleTypeClick} className={"btn btn-default" + (active ? " active" : "")}><span className={"glyphicon glyphicon-" + icon}></span><span className="type">{name}</span></button>;
      }.bind(this));

      var months = data.months.map(function(month) {
        var active = month == this.state.month;
        return <button key={month} type="button" onClick={this.handleMonthClick} className={"col-xs-2 col-md-1 btn btn-default" + (active ? " active" : "")}>{{ month }}</button>;
      }.bind(this));
